...
**All changes to nanousd source code must be accompanied by tests in both test suites and must pass CI.**
...
Tests must be run from `${CMAKE_SOURCE_DIR}` (the repo root) to find test data files in `tests/usda/`, `tests/usdc/`, and `tests/compliance/usda/`. The `CMakeLists.txt` configures this via `WORKING_DIRECTORY ${CMAKE_SOURCE_DIR}` in all `add_test()` calls.

### CI

The GitHub Actions workflow (`.github/workflows/ci.yml`) runs both test suites on every push and pull request, in both Release and Debug configurations.
